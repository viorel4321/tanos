...
public class SystemMessagePacket extends SysMsgContainer<SystemMessagePacket>
{
	...
	{
		if(count > 1)
			return new SystemMessagePacket(SystemMsg.S1_HAS_OBTAINED_S3_S2).addName(target).addItemName(itemId).addLong(count);
		if(enchantLevel > 0)
			return new SystemMessagePacket(SystemMsg.S1_HAS_OBTAINED_S3_S2).addName(target).addInteger(enchantLevel).addItemName(itemId);
		return new SystemMessagePacket(SystemMsg.S1_HAS_OBTAINED_S3_S2).addName(target).addItemName(itemId);
	}
