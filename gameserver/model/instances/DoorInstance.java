...
import l2s.gameserver.network.l2.s2c.DoorInfo;
import l2s.gameserver.network.l2.s2c.DoorStatusUpdate;
...
public class DoorInstance extends Creature
{
	...
	public boolean isAttackable(Creature attacker)
	{
		...
		{
			case WALL:
				if(!attacker.isSummon() || !((Servitor) attacker).isSiegeWeapon() || siegeEvent == null || !siegeEvent.containsSiegeSummon((SummonInstance) attacker))
					return false;
				break;
			case DOOR:
	...
	{
		DoorInfo su = new DoorInfo(this);
		for(Player player : World.getAroundPlayers(this))
			if(player != null)
				player.sendPacket(su, new DoorStatusUpdate(this, isAutoAttackable(player)));
	}
	...
	public List<L2GameServerPacket> addPacketList(final Player forPlayer, final Creature dropper)
	{
		...
		list.add(new DoorInfo(this));
		list.add(new DoorStatusUpdate(this, isAutoAttackable(forPlayer)));
		return list;
	}
